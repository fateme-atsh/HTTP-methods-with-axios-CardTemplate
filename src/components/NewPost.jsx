...
class NewPost extends Component {
    ...
    render () {
            ...
            <section className='grid grid-cols-1 m-10 shadow-xl p-10 text-justify bg-gray-100'>
                ...
                <input type='text' name='title' value={this.state.title} onChange={(event) => this.setState({title: event.target.value})}/>
                <label name='content' className='font-semibold text-lime-800 text-2xl'>Content : </label>
                <textarea name='content' value={this.state.body} onChange={(event) => this.setState({body: event.target.value})}/>
                <div>
